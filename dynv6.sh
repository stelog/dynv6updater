...
if [ -z "$hostname" -o -z "$token" ]; then
  echo "Usage: $0 <hostname> [device] <token>"
  exit 1
fi
...
address_ipv6=$(ip -6 addr list scope global dynamic $device | grep -v fd | grep -Po 'inet6 \K[0-9a-fA-F:]+' | head -n1 | tr -d '\n')
address_ipv4=$(curl -s https://checkip4.spdyn.de)
...
if [ "$update_ipv6" = true ]; then
  echo "Updating IPv6 for $hostname"
  $bin "https://dynv6.com/api/update?hostname=$hostname&ipv6=$address_ipv6&token=$token"
  echo ""
fi

if [ "$update_ipv4" = true ]; then
  echo "Updating IPv4 for $hostname"
  $bin "https://dynv6.com/api/update?hostname=$hostname&ipv4=$address_ipv4&token=$token"
  echo ""
fi

# save current address
echo "$address_ipv6" > "$file_ipv6"
